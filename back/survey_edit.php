    ...
    <div class="col-11" id="options">
        <?php foreach($options as $idx => $option){ ?>
            <div class="option form-group row-col-12">
                ...
                <!-- 這邊後來需要修正唷。 -->
                <a href="./api/survey_option_del.php?id=<?=$option['id']?>" class="btn brn-danger brn-sm py-0">-</a>
                <!-- 減少選項用的。 -->
            </div>
        <?php } ?>
    </div>
    <div id="options" class="col-11">
        <input type="submit" class="btn btn-primary mx-1" value="修改">
        <input type="reset" class="btn btn-warning mx-1" value="重置">
    </div>
</form>
<!-- js jq 這邊看不懂，比需要花時間理解。 -->
<script>
